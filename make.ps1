if ($args.Count -eq 0) {
    ...
    if (-not (Test-Path $filename)) {
        # If file does not exist, check for .c and .cpp extensions and try to find the file
        $filename_c = "$filename.c"
        $filename_cpp = "$filename.cpp"
        if (Test-Path $filename_c) {
            $filename = $filename_c
            Write-Host "Warning: Using '$filename' as input file."
        } elseif (Test-Path $filename_cpp) {
            $filename = $filename_cpp
            Write-Host "Warning: Using '$filename' as input file."
        } else {
            # If neither file exists, print error message and usage message
